...
export const MainContainer = () => {
  ...
  const [dataset, setDataset] = useState(Array(27).fill(0).map((x, i) => "" + i));
  ...
  return (
    <div className="App">
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>{dataset.map((x, i) => <div style={{fontSize: 16, padding: 3}}>{i}({result[i]}) <UseInput type="text" data={dataset} edit={setDataset} index={i} value={x} /></div>)}</div>
